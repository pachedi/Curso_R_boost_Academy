...
## BASE TARJETAS SUBE  
Carguemos ahora la base de usuarios SUBE:
...
```{r }
op <- par(mfrow=c(1, 2))
plot(FEM$DIA_TRANSPORTE, FEM$Cantidad_total, type = 'l', col = 'red', main = 'Movilidad 2020 V vs M')
plot(FEM$DIA_TRANSPORTE, MASC$Cantidad_total,type = 'l', col = 'blue')
```
...
Podemos agregar "hitos" con lineas verticales. Por ejemplo, el comienzo del IFE:
```{r }
...
```
Podemos agregar hacer lo mismo de manera horizontal. Por ejemplo, la media de la cantidad de viajes:
...
ggplot(filtros_genero, aes(x = DIA_TRANSPORTE, y= Cantidad_total, color=GENERO)) + 
  geom_line()+
  geom_vline(aes(xintercept= as.Date("2020-03-20"),col="ASPO"), linetype="dashed")+
  geom_hline(aes(yintercept= mean(Cantidad_total),col="mean"), linetype="dashed")+
  labs(title = 'Movilidad diaria SUBE por Sexo en 2020',
       x = 'Fecha',
       y = 'Cantidad de viajes SUBE',
       caption = 'Elaboracion propia en base a datos abiertos SUBE',
...
Agreguemos colores de manera manual con scale_color_manual()
```{r }
ggplot(filtros_genero, aes(x = DIA_TRANSPORTE, y= Cantidad_total,color=GENERO)) + 
  geom_line()+
  labs(title = 'Movilidad diaria SUBE por Sexo en 2020',
  ...
  geom_vline(aes(xintercept =as.Date('2020-11-08'), col='Fin del IFE') , linetype="dashed")+
  scale_color_manual(name = 'Referencias:', values=c(
    ...
    "F" = 'violet', 
    "M" ='light blue',
    "Sin_registro"="gray"), labels=c("ASPO","DISPO","Fin del IFE","Femenino", "Mascuino", "Sin registro"))
```  
